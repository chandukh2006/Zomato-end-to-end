...
async def get_cart(user_id: str):
    """Get cart items with menu item details and grand total"""
    cart_items = await CartItem.find({"user_id": user_id}).to_list()
...
async def add_item(request: AddItemRequest):
    """Add item to cart. Returns 409 if cart has items from different restaurant."""
    # Get menu item to find restaurant_id
    menu_item = await MenuItem.get(request.menu_item_id)
    if not menu_item:
        raise HTTPException(status_code=404, detail="Menu item not found")
    
    # Check if cart has items from different restaurant
    existing_cart_items = await CartItem.find({"user_id": request.user_id}).to_list()
    
    if existing_cart_items:
        existing_restaurant_id = existing_cart_items[0].restaurant_id
...
async def update_item(cart_id: str, request: UpdateItemRequest):
    """Update cart item quantity. Delete if quantity is 0."""
...
async def clear_cart(user_id: str):
    ...
    await CartItem.find({"user_id": user_id}).delete()
